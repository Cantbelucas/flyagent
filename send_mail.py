...
Kaldes af GitHub Actions efter hver søgning. Rapporten vedhæftes som
rejseplan.html, og de vigtigste tal står i selve mailen.

Alle indstillinger kommer fra GitHub Secrets - der må ALDRIG stå
...
def indstillinger() -> dict | None:
    """Henter opsætningen fra miljøet. Returnerer None hvis noget mangler."""
...
def emne(status: dict) -> str:
    ...
    if pris:
        return f"Flysøgning: {kr(pris)} kr. pr. person"
    return "Flysøgning: ingen rejse opfylder kriterierne"
...
def brevtekst(status: dict) -> tuple[str, str]:
    ...
    linjer.append("")
    linjer.append("Den fulde rapport med alle afgange og købslinks er vedhæftet "
                  "som rejseplan.html - åbn den i en browser.")
    ...
    fald = (f'<p style="margin:0 0 16px;color:#1a7f37;font-weight:600">'
            f'Prisen er faldet {kr(faldet)} kr. siden sidste søgning.</p>') if faldet else ""
  ...
  <p style="margin:0 0 4px;color:#6b6b70;font-size:13px">Flysøgning{f' &middot; {tid}' if tid else ''}</p>
  ...
  <p style="margin:0;padding:12px 14px;background:#f5f5f7;border-radius:8px;font-size:14px">
     Den fulde rapport med alle afgange og købslinks er vedhæftet som
     <strong>rejseplan.html</strong> &ndash; åbn den i en browser.</p>
</div>"""
    return tekst, html
...
def main() -> None:
    ...
    besked.set_content(tekst, charset="utf-8")
    besked.add_alternative(html, subtype="html", charset="utf-8")

    if rapport.exists():
        besked.add_attachment(rapport.read_bytes(), maintype="text", subtype="html",
                              filename="rejseplan.html")
    else:
        print("rejseplan.html findes ikke - sender mailen uden vedhæftning.")

    port = int(n["MAIL_PORT"])
    tls_opsaetning = ssl.create_default_context()
    try:
        if port == 465:
            with smtplib.SMTP_SSL(n["MAIL_SERVER"], port, context=tls_opsaetning, timeout=30) as s:
                s.login(n["MAIL_BRUGER"], n["MAIL_KODE"])
                s.send_message(besked)
        else:
            with smtplib.SMTP(n["MAIL_SERVER"], port, timeout=30) as s:
                s.starttls(context=tls_opsaetning)
                s.login(n["MAIL_BRUGER"], n["MAIL_KODE"])
                s.send_message(besked)
        ...
        print(f"Kunne ikke nå mailserveren: {fejl}", file=sys.stderr)
        raise
